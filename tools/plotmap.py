...
#%%            
    ...
    im_names = np.zeros(num_pro).astype(str)
    for i in range(num_pro):
        im_names[i] = str(i).zfill(6)+'.jpg'
...
#%%
    cars_capacity = np.zeros(camera_total[1].shape[0])
    for i in range(camera_total[1].shape[0]):
        direction = 'location/'+camera_total[0][i]+'/plots'
        if os.path.isfile(os.path.join(cfg.DATA_DIR, direction,'capacity.npy')):
            cars_capacity[i] = np.load(os.path.join(cfg.DATA_DIR, direction,'capacity.npy'))
    camera_total = np.vstack((camera_total,cars_capacity))
...
#    icon = folium.Icon(color="yellow", icon="ok")
#    marker = folium.Marker(location=[lon, lat], popup=popup, icon=icon)
#    schoolMap_test.add_child(marker);
#    
#    
...
#    station = '42'
#    png = 'mpld3_{}.png'.format(station)
#    vehicles = count_vehicles('1 Ave @ 42 S_detections.txt')
#    fps = 3
#    y = print_vehicles_in_time(vehicles,fps)
#    y = pd.Series(y)
#    fig = y.plot.hist(stacked=True, bins=50)
#    fig.savefig(png, dpi=resolution)
#    encoded = base64.b64encode(open(png, 'rb').read())
#    iframe = folium.IFrame(html(encoded), width=(width*resolution)+20, height=(height*resolution)+20)
#    popup = folium.Popup(iframe, max_width=2650)
